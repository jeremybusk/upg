...
DECLARE
  ...
  m monitors;
  url text;
  resp record;                -- <== generic record instead of http_response
  body text;
  ok boolean := false;
  err text := NULL;
  ip_txt text;
...
BEGIN
  ...
  -- Call pgsql-http; capture fields into local scalars (so we don't need the composite type)
  SELECT status, content
    INTO resp_status, resp_content
  FROM http_get(
    url,
    ARRAY[ http_header('Accept','text/plain; version=0.0.4') ],
    s.http_timeout_ms
  );
