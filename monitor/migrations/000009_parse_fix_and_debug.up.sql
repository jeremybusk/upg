...
-- 1) prom_value: strip CRs, allow trailing spaces/comments, CRLF
CREATE OR REPLACE FUNCTION prom_value(body text, metric text)
RETURNS double precision AS $$
DECLARE
  -- ^\s*metric{labels}? <number> [# comment] [CR]
  m text := '^\\s*' || metric ||
            '(?:\\{[^}]*\\})?\\s+([-+]?((\\d+\\.?\\d*)|(\\.?\\d+))(?:[eE][-+]?\\d+)?)\\s*(?:#.*)?\\s*\\r?$';
  v text;
BEGIN
...
-- 2) prom_label: strip CRs, allow leading spaces; no numeric tail required
...
-- 3) run_monitor_once: when 200 but we cannot parse probe_success, annotate a short body head for debugging
  ...
  IF resp_status = 200 THEN
    ...
    IF success_val IS NULL THEN
      ...
      dbg_labels := jsonb_build_object(
        'url', url,
        'body_head', substring(resp_body from 1 for 200)  -- short sample
      );
    ELSE
      ok := (success_val > 0.5);
    END IF;
