...
-- 1) More robust prom_value: allow leading spaces, ignore trailing comments
CREATE OR REPLACE FUNCTION prom_value(body text, metric text)
RETURNS double precision AS $$
DECLARE
  -- ^\s*metric{labels}? <number> [# comment]
  m text := '^\\s*' || metric ||
            '(?:\\{[^}]*\\})?\\s+([-+]?((\\d+\\.?\\d*)|(\\.?\\d+))(?:[eE][-+]?\\d+)?)\\s*(?:#.*)?$';
  v text;
BEGIN
...
-- 2) More robust prom_label: allow leading spaces, don't require the numeric value match
CREATE OR REPLACE FUNCTION prom_label(body text, metric text, label_key text)
RETURNS text AS $$
DECLARE
  -- ^\s*metric{ ... label_key="VALUE" ... }
...
-- 3) Update runner to set a clearer error when parsing fails despite 200
