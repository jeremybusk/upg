...
-- Make run_monitor_once robust to pgsql-http overload differences
...
BEGIN
  ...
  -- Try 3-arg -> 2-arg -> 1-arg http_get, with explicit header array cast
  BEGIN
  ...
  -- Best-effort resolved IP (may be absent depending on module)
  ip_txt := COALESCE(
